...
Small MLP (default 36->128->128->6) trained with MSE on the per-joint residual target produced by
`sage_to_training.py`. Inputs/outputs are normalized using the stats baked into train.npz. The
exported TorchScript model at `models/compensator.pt` loads at deploy WITHOUT Isaac Sim (env C).

The exported module takes a RAW (un-normalized) feature row in `.pos` units and returns a RAW
residual prediction in `.pos` units — normalization is folded INTO the scripted module so the
deployment side (`actuator_compensator.py` / `so101_eval.py`) does not need the stats files.

Run (env C):
  python tools/train_compensator.py --data-dir outputs --epochs 200 --out models/compensator.pt

Reports per-joint RMSE of the residual BEFORE correction (i.e. the raw gap magnitude) vs AFTER
(residual minus model prediction), mirroring SAGE's per-joint metric format.
"""
...
# ---------------------------------------------------------------------------
# Model: normalization folded in, so the scripted artifact is deploy-ready as-is.
# ---------------------------------------------------------------------------
class Compensator(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, hidden: tuple[int, ...],
        ...
        # buffers travel with the scripted module (not trained)
    ...
    def forward(self, x_raw: torch.Tensor) -> torch.Tensor:
        """x_raw: (B, in_dim) in `.pos` units -> residual prediction (B, out_dim) in `.pos` units."""
...
def per_joint_rmse(residual: np.ndarray) -> np.ndarray:
    """RMSE per joint of a residual array [N,6]."""
    return np.sqrt(np.mean(residual ** 2, axis=0))


def main() -> None:
    p = argparse.ArgumentParser(description="Train residual compensator (Phase 3 Task C)")
    p.add_argument("--data-dir", type=Path, default=Path("outputs"))
    ...
    p.add_argument("--device", default="cpu",
                   help="cpu is fine (model is tiny); use cuda:1 to keep GPU0 free for Isaac Sim")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--out", type=Path, default=Path("models/compensator.pt"))
    args = p.parse_args()

    torch.manual_seed(args.seed)
    np.random.seed(args.seed)

    tr = _load_npz(args.data_dir / "train.npz")
    va = _load_npz(args.data_dir / "val.npz")

    meta = json.loads(str(tr["meta"]))
    if meta["feature_dim"] != fs.FEATURE_DIM or meta["use_qdot"] != fs.USE_QDOT:
        raise RuntimeError(
            f"feature_spec mismatch: dataset built with feature_dim={meta['feature_dim']} "
            f"use_qdot={meta['use_qdot']} but current feature_spec has "
            f"feature_dim={fs.FEATURE_DIM} use_qdot={fs.USE_QDOT}. Rebuild dataset or revert spec."
        )
    ...
    rmse_after = per_joint_rmse(Yv_np - pred_v)       # residual the model fails to explain

    print("\nper-joint residual RMSE (.pos units: arm deg, gripper 0..100)")
    ...
    example = torch.zeros(1, fs.FEATURE_DIM)
